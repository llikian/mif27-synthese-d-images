...
bool read_meshio_data( const char *filename, MeshIOData& data )
{
    ...
    for(;;)
    {
        ...
        else if(line[0] == 'f')         // triangle a b c, les sommets sont numerotes a partir de 1 ou de la fin du tableau (< 0)
        {
            ...
            // texcoords et normales pour tout le monde...
            for(unsigned k= 0; k < wp.size(); k++)
            {
                ...
                if(t == -1)
                {
                    if(default_texcoord == -1)
                    {
                        wtexcoords.push_back( Point(0.5, 0.5, 0) ); 
                        default_texcoord= wtexcoords.size(); 
                    }
                    wt[k]= default_texcoord;    
                }
                if(n == -1)
                {
                    if(default_normal == -1)
                    {
                        wnormals.push_back( Vector(0, 0, 0) ); 
                        default_normal= wnormals.size(); 
                    }
                    wn[k]= default_normal; 
                }
            }
    ...
    if(default_texcoord != -1) printf("  using default texcoords...\n");
    if(default_normal != -1) printf("  using default normals...\n");

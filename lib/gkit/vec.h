...
struct Point {
    ...
    explicit Point(
        const vec3& v); // l'implementation se trouve en fin de fichier, la structure vec3 n'est pas encore connue.
    explicit Point(
        const vec4& v); // l'implementation se trouve en fin de fichier, la structure vec3 n'est pas encore connue.
    //! cree un point a partir des coordonnes du vecteur (v.x, v.y, v.z).
    explicit Point(
        const Vector& v); // l'implementation se trouve en fin de fichier, la structure vector n'est pas encore connue.
...
//
inline float Point::operator()(const unsigned int i) const {
    return (&x)[i];
}

inline float Vector::operator()(const unsigned int i) const {
    return (&x)[i];
}

inline float& Point::operator()(const unsigned int i) {
    return (&x)[i];
}

inline float& Vector::operator()(const unsigned int i) {
    return (&x)[i];
}
